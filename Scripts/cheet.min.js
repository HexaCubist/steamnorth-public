...
function addCss(cssString) {
	// Make things a little bit funky
...
// Here we declare an animation to run when the user types iliketrains (at any point)
cheet('i l i k e t r a i n s', function () {
	// Get the train div for later reference
	var train = document.getElementById("train");
	// We'll also try to play a sound effect:
	var audio = new Audio('assets/easter/train.mp3'); // Horn licensed under CC0, so no attribution needed. Modified to be shorter and to fadein/fadeout better
	audio.play();
	setTimeout(function(){
		// Show the parent div
		train.parentElement.style.visibility = "visible";
		// Next, we run the animation after a delay so that the browser can apply above changes, and so that the sound can kick in.
		console.log("Running Train");
		train.style["animation-name"] = "Woosh";
		// Make EVERYTHING animate
		addCss('* { transition: 1s ease-in-out !important; }');
		// Wait for the animation to finish
		setTimeout( function() {
			train.parentElement.style.visibility = "hidden";
			console.log("Removing Train");
			train.style["animation-name"] = "";
			addCss('* { background-image: url("assets/easter/heresjohnny.jpg"); }');
			}, 1100)
	},500);
});
